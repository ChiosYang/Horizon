...
良いニュースは散らばっていて、悪いニュースは尽きることがありません。Horizonは、Hacker News、Reddit、Telegram、RSS、Twitter/X、GitHub、Google News、OpenBBに対する個人的な一次フィルタを提供します。記事を取得・重複排除・スコアリング・フィルタリングし、背景情報やコミュニティでの議論を付加します。
...
- **📡 自分だけの情報源を監視** — Hacker News、RSS、Reddit、Telegram、Twitter/X、GitHubのリリースやユーザーアクティビティ、Google News検索、OpenBBの金融ニュースウォッチリストを1つのパイプラインで追跡
...
- **📝 日次サイトを公開** — 生成されたMarkdownを必要に応じてGitHub Pagesの日次ブリーフィングサイトとして公開
- **📧 メールで配信** — 設定された固定宛先へSMTPで日次ブリーフィングを送信
...
| **Google News** | Google News RSSによるキーワードベースの最新ニュース | — |
...
| チャネル | 内容 |
|---------|--------------|
| **GitHub Pages 日次サイト** | 有効化時に生成されたMarkdownを`docs/`へコピーし、GitHub Pagesで公開します |
| **メール配信** | 設定された固定宛先へSMTPで日次ブリーフィングを送信します |
